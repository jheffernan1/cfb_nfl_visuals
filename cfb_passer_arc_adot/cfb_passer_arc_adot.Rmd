...
```{r}
# 1) Read & rename
raw <- read.csv("passing_summary.csv", header = TRUE) %>%
  rename(team = team_name)

# 2) Identify the directional cols
att_cols  <- grep("_attempts$", names(raw), value = TRUE)
att_cols  <- setdiff(att_cols, "base_attempts")         # drop the lone 'base_attempts'
dot_cols  <- grep("_avg_depth_of_target$", names(raw), value = TRUE)
comp_cols <- grep("_completions$", names(raw), value = TRUE)

# 3) Build same‑sized matrices
att_mat  <- as.matrix(raw[, att_cols])
dot_mat  <- as.matrix(raw[, dot_cols])
comp_mat <- as.matrix(raw[, comp_cols])

# 4) Assemble the slim tutorial df, rounding ADOT to 1 decimal
pass_summary <- tibble::tibble(
  player               = raw$player,
  team                 = raw$team,
  attempts             = raw$base_attempts,
  completions          = rowSums(comp_mat, na.rm = TRUE),
  avg_depth_of_target  = round(
                           rowSums(att_mat * dot_mat, na.rm = TRUE) /
                           rowSums(att_mat, na.rm = TRUE),
                         1
                         )
)
...
magick::image_write(
  plot_with_ncaa,
  path = "/Users/heff/Desktop/jheffernan/cover2figures_tutorials/cfb_passer_arc_adot/cfb_passer_arc_adot.png"
)
```
