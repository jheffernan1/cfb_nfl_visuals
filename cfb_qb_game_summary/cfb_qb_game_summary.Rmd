...
library(patchwork)
library(showtext)
library(ragg)       # for high‑res PNG
library(magick)     # for add_logo
```
...
download.file(
  url      = "https://raw.githubusercontent.com/mlfurman3/gg_field/main/gg_field.R",
  destfile = "gg_field.R",
  method   = "curl"      # or method="auto" on Windows
)
source("gg_field.R")
```
...
# Pull fresh Week 13 data:
pbp_new <- cfbd_pbp_data(year = 2022, week = week, epa_wpa = TRUE)
...
pbp_rp <- pbp %>%
  ...
  mutate(
    player_name = coalesce(passer_player_name,
                           rusher_player_name,
                           fumble_player_name)
  ) %>%
  filter(player_name %in% qbs$full_name) %>%
  mutate(fumble_vec = if_else(fumble_player_name != player_name, 0, as.numeric(fumble_vec)))
...
box <- pbp_rp %>%
  ...
  summarise(
    tot_epa  = round(sum(EPA, na.rm = TRUE), 2),
    pass_epa = round(mean(EPA[pass == 1], na.rm = TRUE), 2),
    epa_lost = round(
      sum(
        if_else(
          int == 1 | sack == 1 | fumble_vec == 1,
          EPA,
          0
        ),
        na.rm = TRUE
  ...
  tab_style(
    style     = list(cell_fill(color = "#070F6C"), cell_text(color = "white")),
    locations = cells_title()
  ) %>%
  tab_style(
    style     = cell_fill(color = "#F0F8FF"),
    locations = list(cells_column_labels(), cells_body(rows = everything()))
  ) %>%
  tab_style(
...
att_long <- att %>%
  ...
  mutate(att = na_if(att, 0))   # Only turn zeros in the att column into NA
...
magick::image_write(
  plot_with_box,
  path = "/Users/heff/Desktop/jheffernan/cover2figures_tutorials/cfb_qb_game_summary/cfb_qb_game_summary.png"
)
```
