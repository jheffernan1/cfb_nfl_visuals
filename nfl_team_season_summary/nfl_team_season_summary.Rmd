...
opp_epa <- pbp %>%
  ...
  summarise(offense = round(mean(epa), 2), .groups = "drop") %>%   # add .groups if you like
  left_join(
    pbp %>%
      filter(rush == 1 | pass == 1) %>%
      group_by(defteam, week, away_team) %>%
      summarise(defense = round(mean(epa), 2), .groups = "drop"),
    by = c("week", "posteam" = "defteam")
  ) %>%
  # now continue on as before
  mutate(
  ...
  arrange(team, week) %>%
  pivot_longer(
    cols = -c(team, week, opponent),
    names_to = "unit",
    values_to = "epa"
  ) %>%
  mutate(
    label    = ifelse(epa >= 0, paste0("+", epa), as.character(epa)),
    text_col = case_when(
      unit == "offense" & epa >= 0 ~ "darkgreen",
      unit == "defense" & epa <  0 ~ "darkgreen",
      TRUE                         ~ "darkred"
    )
  )
...
epa_dat <- pbp %>%
  ...
  mutate(
    across(c(def_pass_epa, def_rush_epa), f1),  # only invert defense
...
p5 <- ggplot(epa_dat,
  ...
  labs(subtitle = "EPA/Pass Rank vs. NFL") +
  geom_richtext(aes(x = Inf, y = -Inf,
                    label = glue("EPA/Pass: {round(team_epa$def_pass_epa, 2)}<br><br>Better than {def_pass_epa_pct}%<br>of NFL teams")),
                fill = "#F0F8FF", label.colour = "#F0F8FF",
...
p6 <- ggplot(epa_dat,
  ...
  labs(subtitle = "EPA/Rush Rank vs. NFL") +
  geom_richtext(aes(x = Inf, y = -Inf,
                    label = glue("EPA/Rush: {round(team_epa$def_rush_epa, 2)}<br><br>Better than {def_rush_epa_pct}%<br>of NFL teams")),
                fill = "#F0F8FF", label.colour = "#F0F8FF",
                size = 5, hjust = 1, vjust = 1, family = "Fjalla One") +
...
```{r}
### Polar Plots ###
f2 <- function(x) rank(-x, ties.method = "min")  # highest = best
f3 <- function(x) rank(x,  ties.method = "min")  # lowest  = best

polar_dat <- pbp %>%
  # offense throughput & efficiency
  filter(rush == 1 | pass == 1) %>%
  group_by(posteam) %>%
  summarise(
    ypp           = sum(yards_gained, na.rm = TRUE) / n(),
  ...
  left_join(
    pbp %>%
      ...
      summarise(pts = max(posteam_score_post), .groups = "drop") %>%
      group_by(posteam) %>%
      summarise(avg_pts = mean(pts), .groups = "drop"),
    by = "posteam"
  ) %>%
  # defense throughput & efficiency
  left_join(
    pbp %>%
      filter(rush == 1 | pass == 1) %>%
      group_by(defteam) %>%
      summarise(
        def_ypp           = sum(yards_gained, na.rm = TRUE) / n(),
        def_success_rate  = mean(success),
        def_third_success = mean(success[down == 3], na.rm = TRUE),
        def_pass_exp      = sum(pass == 1 & yards_gained >= 15, na.rm = TRUE),
        def_rush_exp      = sum(rush == 1 & yards_gained >= 10, na.rm = TRUE),
  ...
  # average points per game (defense)
  left_join(
    pbp %>%
      group_by(defteam, week) %>%
      summarise(def_pts = max(posteam_score_post), .groups = "drop") %>%
      group_by(defteam) %>%
      summarise(avg_def_pts = mean(def_pts), .groups = "drop"),
    by = c("posteam" = "defteam")
  ) %>%
  ungroup() %>%
  select(-c(5:6, 13:14)) %>%
  # apply your ranking functions
  mutate(
    across(c(2:5, 7, 12), f2),
    across(c(6, 8:11, 13), f3)
  ) %>%
  rename(team = posteam)
...
magick::image_write(
  plot_with_ncaa,
  path = "/Users/heff/Desktop/jheffernan/cover2figures_tutorials/nfl_team_season_summary/nfl_team_season_summary.png"
)
```
