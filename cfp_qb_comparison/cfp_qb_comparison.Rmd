...
pbp_rp <- pbp %>%
  ...
  mutate(player_name = coalesce(rusher_player_name,
                                passer_player_name,
                                fumble_player_name)) %>%
  left_join(
    qbs_unique,
    by = c("player_name" = "full_name", "pos_team" = "team"),
...
base <- ggplot(plot_df,
  ...
  geom_label(aes(y = pct_rank * 0.60,
                 label = pct_rank),
               size = 4,
               family = "Fjalla One",
               fontface = "bold") +
...
magick::image_write(
  plot_with_ncaa,
  path = "/Users/heff/Desktop/jheffernan/cover2figures_tutorials/cfp_qb_comparison/cfp_qb_ranks.png"
)
```
