---
title: "cfb_qb_wr_connections2.Rmd"
author: "Heff"
date: "2025-04-29"
...
pbp_p <- pbp %>%
  filter(pass == 1, !is.na(EPA), is.na(penalty_detail),
         pos_team %in% team_info$team) %>%
  filter(!is.na(passer_player_name)) %>%        # drop any weird NA QBs
  mutate(passer = passer_player_name)           # use the true QB name

df <- pbp_p %>%
  group_by(passer, receiver = receiver_player_name, team = pos_team) %>%
  summarise(
  ...
  filter(comp >= 35) %>%       # adjust here if your max_comp < 35
  arrange(-epa) %>%
  mutate(text_label = paste0(passer, " to ", receiver, " (", comp, ")"))
...
magick::image_write(plot_with_ncaa, "/Users/heff/Desktop/jheffernan/cover2figures_tutorials/cfb_qb_wr_connections/cfb_qb_wr_pairs_p5.png")
...
magick::image_write(plot_with_ncaa, "/Users/heff/Desktop/jheffernan/cover2figures_tutorials/cfb_qb_wr_connections/cfb_qb_wr_pairs_g5.png")
```
