...
```{r}
df <- pbp %>% 
  select(pos_team, game_id, drive_number, drive_result, drive_play_number, down, yards_to_goal) %>%
  rename(play_number = drive_play_number)

red_zone <- df %>%
  filter(down == 1 & yards_to_goal <= 20) %>%
  group_by(pos_team, game_id, drive_number) %>%
  slice_min(play_number) %>%
  mutate(td = as.integer(drive_result == "TD")) %>%
  group_by(pos_team) %>%
  summarise(n = n(), td_rate = sum(td)/n()) %>%
  mutate(td_rate = round(td_rate*100, 1),
         label   = as.character(td_rate)) %>%
  rename(team = pos_team) %>%
  left_join(team_info, by = "team") %>%
  mutate(color = case_when(
    team == "Charlotte"      ~ "#046a38",
    team == "Toledo"         ~ "#15397f",
    TRUE                     ~ color
  )) %>%
  arrange(-td_rate)
...
magick::image_write(
  plot_with_ncaa,
  path = "/Users/heff/Desktop/jheffernan/cover2figures_tutorials/cfb_red_zone/cfb_red_zone_p5_top.png"
)
...
base <- ggplot(g5_top, aes(x = reorder(team, -td_rate), y = td_rate,
  ...
  theme(plot.background = element_rect(fill = "#F0F8FF", colour = "#F0F8FF"),
        ...
        panel.grid.major.x = element_blank(),
        panel.grid.minor = element_blank()) +
  labs(title = "Converting Red Zone Trips to Touchdowns: Weeks 0-7",
...
magick::image_write(
  plot_with_ncaa,
  path = "/Users/heff/Desktop/jheffernan/cover2figures_tutorials/cfb_red_zone/cfb_red_zone_g5_top.png"
)
```
